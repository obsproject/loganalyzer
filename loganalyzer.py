...
def checkGPU(lines):
    ...
    if(len(d3dAdapter) > 0):
        if(len(adapters) == 2 and ('Intel' in d3dAdapter[0])):
            return [3, "Wrong GPU", """Your Laptop has two GPUs. OBS is running on the weak integrated Intel GPU. For better performance as well as game capture being available you should run OBS on the dedicated GPU. Check the <a href="https://obsproject.com/wiki/Laptop-Performance-Issues">Laptop Troubleshooting Guide</a>."""]
...
def check32bitOn64bit(lines):
    ...
    if(('64-bit' in winVersion[0]) and (('64-bit' not in obsVersion[0]) and ('64bit' not in obsVersion[0]))):
...
def checkEncoding(lines):
    ...
    if(val != 0):
        if(val >= 15):
            ...
            severity = 1
        return [severity, "{}% Encoder Overload".format(val), """The encoder is skipping frames because of CPU overload. Read about <a href="https://obsproject.com/wiki/General-Performance-and-Encoding-Issues">General Performance and Encoding Issues</a>"""]
