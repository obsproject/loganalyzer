...
class ObsVersion():
    ...
    Follows the semver 2.0.0 spec at https://semver.org/ with the single exception that
    pre-release version MUST have the following formatting : `<type><patch>`
    with `<type>` being one of `alpha`, `beta`, or `rc`
    and `<patch>` (optional) being any number of digits
    """
    _nan = float('nan')
